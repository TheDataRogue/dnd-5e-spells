...
def class_spells(data,char_class:str):
    ...
    the argument.
    """
    classes_list = ['Artificer','Bard','Cleric','Druid','Paladin','Ranger',
                    'Sorcerer','Warlock','Wizard']
    if char_class in classes_list:
        char_class_spells = [] # creates an empty list
        for i in range(0,len(data)): # with the next line, iterates through the entire DataFrame by index
            name = data['name'][i] # creates a string object equal to the name of the spell 
            spell = data[data['name'] == name] # creates a single DataFrame object using the name of the spell as a filter
        ...
        raise NameError("Not a spellcasting class from D&D 5E.")
